...
export default function CommunityBanner({ variant = "chip" }: CommunityBannerProps) {
  if (variant === "chip") {
    return (
      <Link
        ...
        className="inline-flex items-center gap-2 px-4 py-2 rounded-full border border-cfn-cream/30 text-cfn-cream/80 hover:text-cfn-cream hover:border-cfn-cream/60 text-sm transition-colors group"
      >
        <Users className="w-4 h-4 group-hover:text-cfn-cream transition-colors" />
  ...
  return (
    <div className="bg-cfn-navy rounded-2xl p-8 text-center text-cfn-cream">
      <div className="w-12 h-12 rounded-full bg-cfn-cream/15 flex items-center justify-center mx-auto mb-4">
        <Users className="w-6 h-6 text-cfn-cream" />
      </div>
